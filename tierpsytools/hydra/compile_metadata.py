...
def merge_robot_metadata (sourceplates_file,saveto=None,del_if_exists=False):
    ...
    return:
        robot_metadata: pandas dataframe
            Robot related metadata for the given day of experiments as
            dataframe
    """
    ...
    for n_log,log_file in enumerate(sourceplates['robot_runlog_filename'].unique()):
        ...
        # get unique imaging_plate_id - IB update: specify 02 decimal format
        out_meta['imaging_plate_id'] = out_meta[['source_plate_id', 'destination_slot']].apply(lambda x: 'rr{0:02d}_sp{1:02d}_ds{2:02d}'.format(n_log+1,*x),axis=1)
